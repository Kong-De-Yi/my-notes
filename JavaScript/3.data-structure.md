...
- set.keys()：返回所有 value 的可迭代对象
- set.values()：返回所有 value 的可迭代对象
- set.entries()：返回所有[value,value]的可迭代对象（for...of 默认使用）
...
- 不会阻止 key object 的垃圾回收（当 key object 没有其他引用时，会从内存和 weakMap 中清除）
- 不支持迭代，没有 keys()、values()、entries()方法（因为回收时机不确定）
